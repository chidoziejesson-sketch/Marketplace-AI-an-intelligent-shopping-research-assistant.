...
// FIX: The `ai` instance needs to be accessible by `generateProductImage`. Declare it in the module scope.
...
async function generateProductImage(imagePrompt: string): Promise<ImageResult> {
    if (!ai) {
        throw new Error("Gemini service is not initialized. Check your API_KEY.");
    }
    try {
...
export async function generateMarketplaceResponse(userInput: string): Promise<ApiResponse> {
    ...
    const textResponseResult = await chat.sendMessage({ message: userInput });
    ...
    const imagePromises = rawResponse.products.map(product => 
        generateProductImage(product.image_prompt).then(imageResult => ({ product, imageResult }))
    );

    const results = await Promise.all(imagePromises);

    for (const { product, imageResult } of results) {
