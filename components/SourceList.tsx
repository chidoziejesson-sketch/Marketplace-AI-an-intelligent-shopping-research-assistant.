    ...
    <div className="bg-slate-800/50 p-4 rounded-lg">
      <ul className="space-y-4">
        {sources.map((source, index) => (
          <li key={index} className="p-4 bg-slate-800 rounded-md border-l-4 border-violet-500">
            <div className="flex justify-between items-center">
              <div>
                <h4 className="text-lg font-semibold text-slate-100">{source.platform}</h4>
                <p className="text-sm text-slate-400">{source.region}</p>
              </div>
              <a 
                href={source.url} 
                target="_blank" 
